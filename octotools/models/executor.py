...
from octotools.models.variable_registry import variable_registry
...
class Executor:
    ...
    def register_variable(self, var_name: str, var_value: Any, source_tool: str):
        """注册变量到全局池"""
        self.global_variables[var_name] = var_value
        if self.verbose:
            print(f"✅ Registered variable '{var_name}' from {source_tool}")

    def _extract_and_register_variables(self, context: dict, tool_name: str):
        """从执行上下文中提取标准变量并注册到全局池"""
        expected_outputs = variable_registry.get_tool_outputs(tool_name)
        
        for var_name in expected_outputs:
            if var_name in context and var_name not in ['tool', 'execution']:
                self.register_variable(var_name, context[var_name], tool_name)
...
Remember: Your response MUST end with the Generated Command, which should be valid Python code including any necessary data preparation steps and one or more `execution = tool.execute(` calls, without any additional explanatory text. The format `execution = tool.execute` must be strictly followed, and the last line must begin with `execution = tool.execute` to capture the final output."""
    ...
    def execute_tool_command(self, tool_name: str, command: str) -> Any:
        ...
        def execute_with_timeout(block: str, local_context: dict) -> Optional[str]:
            ...
            enhanced_context = {
                'tool': tool,
                **self.global_variables,  # 注入所有已有的全局变量
                **local_context
            }
            ...
            try:
                # Execute the block in the enhanced context
                exec(block, globals(), enhanced_context)
                result = enhanced_context.get('execution')
                
                # 执行后，提取并注册新生成的标准变量
                self._extract_and_register_variables(enhanced_context, tool_name)
        ...
        try:
            ...
            for block in command_blocks:
                ...
                # Execute the block with timeout protection and variable injection
